...
export async function POST(request: NextRequest) {
  ...
  await new ProviderConnectionService().connect(userIdOrResponse, parsed.data.provider);

  return NextResponse.json({ success: true, provider: parsed.data.provider, connected: true });
}
